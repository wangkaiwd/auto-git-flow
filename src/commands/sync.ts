...
export async function syncAction() {
  ...
  try {
    ...
    if (!isBehind) {
      logger.success(`当前分支 ${currentBranch} 已包含 ${baseBranch.name} 的所有提交，无需同步。`);
      logger.footer();
      return;
    }
    ...
    logger.success(`成功！${baseBranch.name} 已合并至 ${currentBranch} 并推送到 origin。`);
    logger.footer();
    ...
    if (nowBranch !== currentBranch) {
      logger.warn(`流程中断，由于代码冲突或其他错误，请手动处理后切回 ${currentBranch}。`);
    }
    logger.footer();
  }
}
